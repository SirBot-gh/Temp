...
-----
  ...
  Sanitize-DeviceState.ps1 Sanitize whole device state bundles: redact secrets
                           in every XML file and write upload-safe .txt copies
                           (paths preserved). Use when you need full configs
                           as .txt rather than split fragments. $RedactElements
                           is shared with Split-PanBundle.ps1.

  tests\Run-Tests.ps1      Validates Sanitize-DeviceState.ps1 against fixtures
                           in tests\fixtures\.
...
STEP 1 - Prove the tool works (5 minutes)
  ...
  EXPECTED_RESULTS.txt. Expect 21 fragments written, 57 NOT-PRESENT,
